...
from agent.llm import ProviderRegistry
from agent.prompt_builder import FIRST_COMPRESSION_PROMPT, SECOND_COMPRESSION_PROMPT
...
def get_model_context_length(
    ...
    # todo 正常应该是调用模型，然后缓存这些结果。暂时先hard code
...
class ContextCompressor:
    ...
    def _invoke_llm(self, messages: List[Dict[str, Any]]) -> Decision:
    ...
    def _generate_summary(self, turns_to_summarize: List[Dict[str, Any]]) -> Optional[str]:
        ...
        if self._previous_summary:
            ...
            prompt = SECOND_COMPRESSION_PROMPT.format(previous_summary=self._previous_summary,
                                                      content_to_summarize=content_to_summarize,
                                                      summary_budget=summary_budget)
        else:
            # First compaction: summarize from scratch
            prompt = FIRST_COMPRESSION_PROMPT.format(content_to_summarize=content_to_summarize,
                                                     summary_budget=summary_budget)
        ...
        try:
            ...
            response = self._invoke_llm(**call_kwargs)
    ...
    def _find_tail_cut_by_tokens(
        ...
        for i in range(n - 1, head_end - 1, -1):
            ...
            # include reasoning content in estimate(deepseek provider adapter)
            reasoning_content = msg.get("reasoning_content") or ""
            msg_tokens += len(reasoning_content) // _CHARS_PER_TOKEN + 10  # +10 for role/metadata
